...
function Login() {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full rounded-2xl p-8 border border-slate-700/40 shadow-2xl" style={{ background: 'var(--glass-bg)', backdropFilter: 'var(--glass-blur)' }}>
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-extrabold text-slate-100 tracking-tight">Login to FunSpot</h2>
          <p className="mt-2 text-sm text-slate-400">Enter your credentials to access your account</p>
        </div>

        {error && <div className="p-4 mb-6 rounded-lg bg-red-500/10 border border-red-500/20 text-red-300 font-medium text-sm flex items-center gap-2">
          <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          {error}
        </div>}

        {location.state?.redirectTo && (
          <div className="p-4 mb-6 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 font-medium text-sm flex items-center gap-2">
        ...
        <div className="mt-8 pt-6 border-t border-slate-700/50 flex flex-col items-center gap-4 text-sm">
          <p className="text-slate-400">
            Don't have an account? <Link to="/register" className="font-semibold text-emerald-400 hover:text-emerald-300 transition-colors">Register</Link>
          </p>
